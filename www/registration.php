...
<h1>Регистрация</h1>
<?php if (@$ok_capture == 0 or @$number_user != 0 ) {?>
<div id="oshFormReg" style="display: block;">
<?php
    if ($ok_capture == 0) {
        echo 'Неправильно введён код с картинки! ';
    }
    if ($number_user != 0 and $ok_capture == 1) {
        echo 'Такой почтовый почтовый адрес уже зарегистрирован!';
    }
    if ($ok_capture == 1 and $number_user == 0) {
        echo 'Регистрация прошла успешно! Подтверждение регистрации выслано вам на вашу электроную почту.';
        $_POST['e-mail'] = '';
        $_POST['name']   = '';
    }
}
...
<tr>
<td>Имя:</td><td><input type="text" name="name" class="input_enry_form" id="nameReg" value="<?= @$_POST['name']; ?>" /></td><td id="errorName"></td>
</tr>
<tr>
<td>E-mail:</td><td><input type="text" name="e-mail" value="<?= @$_POST['e-mail']; ?>" class="input_enry_form" id="mailReg" /></td><td id="errorMail"></td>
</tr>
<tr>
